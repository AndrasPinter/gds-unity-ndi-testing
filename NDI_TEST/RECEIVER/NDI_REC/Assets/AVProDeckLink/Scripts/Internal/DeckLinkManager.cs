                ...
                if (numInputModes > 0 || numOutputModes > 0)
                {
                    ...
                    if (_logDeviceEnumeration)
                    {
                        ...
                        for (int modeIndex = 0; modeIndex < device.NumInputModes; modeIndex++)
                        {
                            ...
                            Debug.Log("[AVProDeckLink]\t\tInput Mode" + modeIndex + ":  " + mode.ModeDescription + " " + mode.Width + "x" + mode.Height + " @" + mode.FrameRate + " (" + mode.PixelFormatDescription + ") " + (mode.SupportStereo3D ? "Stereo":""));
                        }
                        for (int modeIndex = 0; modeIndex < device.NumOutputModes; modeIndex++)
                        {
                            DeviceMode mode = device.GetOutputMode(modeIndex);
                            Debug.Log("[AVProDeckLink]\t\tOutput Mode" + modeIndex + ": " + mode.ModeDescription + " " + mode.Width + "x" + mode.Height + " @" + mode.FrameRate + " (" + mode.PixelFormatDescription + ") " + (mode.SupportStereo3D ? "Stereo":""));
                        }
                    }
                }
            }
